...
class DroneType(Enum):
    ...
    Alpha is the authoritative v1 police deployment platform. Beta is
    retained only for legacy compatibility with older experiments.
    """
    ALPHA = auto()  # High-altitude sector patrol + self-confirmation
    BETA = auto()   # Legacy compatibility tier
...
class DroneState:
    ...
    mission_state: str = "PATROL_HIGH"
    inspection_state: str = "idle"
    sector_backfill_state: str = "normal"
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'sector_backfill_state': self.sector_backfill_state,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> DroneState:
        ...
        return cls(
            ...
            mission_state=data.get('mission_state', 'PATROL_HIGH'),
            inspection_state=data.get('inspection_state', 'idle'),
            sector_backfill_state=data.get('sector_backfill_state', 'normal'),
...
class SensorType(Enum):
    ...
    WIDE_RGB_CAMERA = auto()      # Wide patrol camera for overhead awareness
    ZOOM_EO_CAMERA = auto()       # Narrow-FOV EO confirmation camera
    THERMAL_CAMERA = auto()       # LWIR thermal imaging
    LIDAR_3D = auto()             # 3D point-cloud sensing for Alpha drones
    RGB_CAMERA = WIDE_RGB_CAMERA  # Backwards-compatible alias
...
class ThreatStatus(Enum):
    ...
    PENDING_CONFIRMATION = auto()  # Confidence above threshold, awaiting inspection
    CONFIRMING = auto()            # Inspector drone en route / on scene
    CONFIRMED = auto()             # Inspector confirmed threat
    CLEARED = auto()               # Inspector cleared false positive
    RESOLVED = auto()              # Threat handled / aged out


class InspectionRecommendation(Enum):
    """Recommended response generated by the mission-policy layer."""
    CONTINUE_PATROL = auto()
    TRACK_HIGH = auto()
    DESCEND_CONFIRM = auto()
    FACADE_SCAN = auto()
    TARGET_CONFIRM = auto()
    CROWD_OVERWATCH = auto()
    HOLD_POSITION = auto()
    REASCEND_REJOIN = auto()
    ABORT = auto()


class DroneMissionState(Enum):
    """Authoritative alpha-only mission state machine."""
    PATROL_HIGH = auto()
    TRACK_HIGH = auto()
...
class AutonomyDecisionType(Enum):
    """Concrete action selected by the mission-policy layer."""
    CONTINUE_PATROL = auto()
    HOLD = auto()
    ASSIGN_INSPECTOR = auto()
    DESCEND = auto()
    EXECUTE_FACADE_SCAN = auto()
    CONFIRM_TARGET = auto()
    REASCEND = auto()
    CROWD_RETASK = auto()
    ABORT = auto()
...
class Threat:
    ...
    confirmed_by: int = -1        # Inspector drone_id
    assigned_beta: int = -1       # Legacy field, mirrors assigned_inspector when used
    assigned_inspector: int = -1  # Alpha inspector assigned
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'assigned_inspector': self.assigned_inspector,
...
@dataclass
class ThreatVector:
    """Fused mission-policy view of a tracked threat."""
    threat_id: str
    ...
    confidence: float
    threat_score: float
    sensor_evidence: List[SensorType] = field(default_factory=list)
    crowd_risk_score: float = 0.0
    spatial_urgency: float = 0.0
    behavioral_urgency: float = 0.0
    inspection_recommendation: InspectionRecommendation = InspectionRecommendation.CONTINUE_PATROL
    mission_profile: str = "crowd_event"


@dataclass
class InspectionPlan:
    """Planned close-inspection behavior for a selected Alpha."""
    threat_id: str
    inspector_id: int
    ...
    safe_altitude: float
    standoff_distance: float = 30.0
    scan_waypoints: List[Waypoint] = field(default_factory=list)
    corridor_safe: bool = True
    approved_by_swarm: bool = False


@dataclass
class SectorCoverageState:
    """Coverage state for a drone-owned hex sector."""
    drone_id: int
    sector_id: str
    coverage_percent: float = 0.0
    degraded: bool = False
    backfilled_by: List[int] = field(default_factory=list)
    active_inspector: Optional[int] = None


@dataclass
class CrowdRiskState:
    """Mission-policy summary for a crowd hotspot."""
    zone_id: str
    center: Vector3
    density_level: str
    stampede_risk: float
    ...
    recommended_action: InspectionRecommendation = InspectionRecommendation.CROWD_OVERWATCH


@dataclass
class AutonomyDecision:
    """Deterministic decision emitted by the mission-policy layer."""
